<script>
    ...
    const handleDelete = async (batchLog) => {
        let index = items.indexOf(batchLog);

        const response = await del(`/resources/inventory/batch-logs/${batchLog.id}`);
    ...
    $: paginatedItems = search ?
        items.filter(i => {
            return i.affectedItemsSKUs.match(`${search}.*`)
...
<div>
    <div class="pb-2">
        ...
        <input type="search" placeholder="Type SKU's" id="filter" bind:value={search} class="input input-sm input-bordered max-w-xs"/>
    </div>
        ...
        <tbody>
        {#each paginatedItems as batchLog}
            <tr>
                <td>{batchLog.id}</td>
                <td>{batchLog.affectedItemsSKUs}</td>
                <td>{batchLog.employeeId}</td>
                <td>
                    {#if !batchLog.note}
                        No Data
                    {:else }
                ...
                <td>
                    <button class="btn btn-sm btn-outline btn-ghost" on:click={handleDelete(batchLog)}>
                        Delete
                    </button>
                </td>
            </tr>
